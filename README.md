...
將 `E:\` 內台北市、新北市的每月 Access 請款報表，依「借車時間」聚合成每日、每小時、每站的使用量與逐站流入／流出資料。資料按月拆檔，新增月份不會重建或放大其他月份的公開資料。
...
public/
  ...
  months/YYYY-MM/
    ...
    flows/<stationId>.json         # 該月逐站流入／流出
...
所有包含來源表名與核對筆數的 audit 都位於 `work/`，不寫到公開頁。
...
- 公開資料只有交易筆數 metric；每個 cell 固定為 `[stationIndex, 0, count]`，不帶補助金額。
- 資料是「該月請款報表中，借車時間落在該月的交易」，不是該月完整全體騎乘；該月借車但之後月份才入帳者不在本批報表。
