...
$n = @{
  ...
  stationId = S @(0x501F,0x8ECA,0x7AD9,0x4EE3,0x865F)
  stationName = S @(0x501F,0x8ECA,0x5834,0x7AD9)
}
...
try {
  ...
  foreach ($table in $allTables) {
    ...
    if (($columns -notcontains $n.activityAt) -or ($columns -notcontains $n.stationId) -or ($columns -notcontains $n.stationName)) {
      $excluded += [ordered]@{table=$table;reason='missing-required-columns'}; continue
    }
    $selected += [ordered]@{Name=$table;Columns=$columns}
  }
  if ($selected.Count -eq 0) { throw "No eligible claim table found in $Path" }
  ...
  foreach ($item in $selected) {
    ...
    $sql = 'SELECT Left(' + (Q $n.activityAt) + ',10) AS activity_date, Mid(' + (Q $n.activityAt) + ',12,2) AS activity_hour, ' +
      (Q $n.stationId) + ' AS station_id, ' + (Q $n.stationName) + ' AS station_name, Count(*) AS claim_count FROM ' +
      (Q $table) + ' WHERE ' + (Q $n.activityAt) + " Like '$Period/%' " +
      'GROUP BY Left(' + (Q $n.activityAt) + ',10), Mid(' + (Q $n.activityAt) + ',12,2), ' + (Q $n.stationId) + ', ' + (Q $n.stationName)
    $cmd = $connection.CreateCommand(); $cmd.CommandTimeout = 0; $cmd.CommandText = $sql
    $reader = $cmd.ExecuteReader(); $tableRows = 0L
    try {
      while ($reader.Read()) {
        $date = ([string]$reader['activity_date']).Replace('/','-'); $hour = [int]([string]$reader['activity_hour'])
        $stationId = Normalize-StationId ([string]$reader['station_id']); $stationName = [string]$reader['station_name']
        $count = [long]$reader['claim_count']
        $key = $date+'|'+$hour+'|'+$stationId+'|'+$stationName
        if (-not $groups.ContainsKey($key)) { $groups[$key] = [ordered]@{date=$date;hour=$hour;stationId=$stationId;stationName=$stationName;count=0L} }
        $groups[$key].count += $count; $tableRows += $count
      }
    } finally { $reader.Close() }
    $tableAudit += [ordered]@{table=$table;columns=$columns.Count;sourceRows=$sourceRows;rows=$tableRows;excludedActivityOutsidePeriod=($sourceRows-$tableRows)}
  }
  $result = [ordered]@{source=$Path;city=$City;period=$Period.Replace('/','-');tables=$tableAudit;excludedTables=$excluded;groups=@($groups.Values)}
  $directory = Split-Path -Parent $Output; if ($directory) {[IO.Directory]::CreateDirectory($directory)|Out-Null}
  [IO.File]::WriteAllText($Output,($result|ConvertTo-Json -Depth 8 -Compress),(New-Object Text.UTF8Encoding($false)))
  Write-Output ("DONE`t$Path`t$($result.groups.Count) groups")
